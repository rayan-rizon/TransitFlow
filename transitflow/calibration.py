...
class PosteriorAffineCalibration:
    ...
    def apply(self, samples_std: np.ndarray, center_std: np.ndarray) -> np.ndarray:
        ...
        latent = calibrated_center + self.scale * (arr - center)
    ...
    def inverse(self, calibrated_std: np.ndarray, center_std: np.ndarray) -> np.ndarray:
        ...
        return center + (latent - calibrated_center) / self.scale

    def log_abs_det_at(self, calibrated_std: np.ndarray) -> np.ndarray:
        ...
        normalized = np.clip(
            (arr - midpoint) / half_range, -1.0 + 1e-12, 1.0 - 1e-12)
        per_dim = (np.log(self.scale) + np.log(half_range)
                   + np.log1p(-(normalized * normalized)))
        return np.sum(per_dim, axis=-1)

    def to_dict(self) -> dict:
        return {
            "schema_version": 2 if self.space == "bounded_tanh" else 1,
...
def _fit_bounded_rank_calibration(
    ...
    """Fit a bounded exact-density map directly against SBC rank shape."""
    ...
    for d in range(dim):
        ...
        design = np.column_stack([np.ones(theta.shape[0]), center[:, d]])
        ...
        def objective(params: np.ndarray) -> float:
            ...
            candidate_scale = float(np.exp(log_scale))
            threshold = center[:, d] + (
                truth_latent[:, d]
                - (candidate_offset + candidate_slope * center[:, d])
            ) / candidate_scale
        ...
        threshold = center[:, d] + (
            truth_latent[:, d] - (offset[d] + slope[d] * center[:, d])
        ) / scale[d]
